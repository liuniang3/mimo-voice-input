...
- 设置界面提供两个明确的转写模式按钮：`Stable` 和 `Fast`
...
设置界面已经把原来的下拉框改成两个独立按钮：

- `Stable`：两段式模式。第一次 MiMo 只做原始音频转写，第二次 MiMo 只处理纯文本，负责删除填充词、合并重复片段和修正标点。
- `Fast`：快速模式。只调用一次 MiMo，延迟更低，但稳定性弱一些。

两个模式之间做了独立性处理：每次录音开始前，程序会从主进程刷新最新设置，并锁定本次录音使用的转写模式。录音或转写过程中再切换模式，只会影响下一次录音，不会改变当前正在处理的这段音频。主进程里也把两条路径拆成了 `callMimoStable()` 和 `callMimoFast()`，后续调整提示词或错误处理时，不容易互相污染。
...
每次录音都会使用录音开始时锁定的模式快照，因此当前录音不会被后续模式切换影响。
